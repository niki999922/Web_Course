...
public class UserCredentialsEnterValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        if (!errors.hasErrors()) {
            ...
            User enterUser  = userService.findByLoginAndPassword(registerForm.getLogin(), registerForm.getPassword());
            if (enterUser == null) {
                errors.rejectValue("password", "invalid.login.or.password", "invalid login or password");
                return;
            }
            if (enterUser.isDisable()) {
                errors.rejectValue("password","invalid.login.or.password", "Вы не можете зайти под эту учётную запись.");
            }
        }
    }
}
